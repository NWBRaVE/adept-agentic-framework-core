...
class MCPStdioWrapper:
    ...
    async def initialize(self):
        ...
        if not self.mcp_session_id:
            ...
            logger.info(f"Generated new MCP Session ID: {self.mcp_session_id}")
        logger.info(f"Initialized wrapper with MCP Session ID: {self.mcp_session_id}")
    async def cleanup(self):
        """Cleanup resources"""
        if self.session:
            await self.session.close()
    
    async def _handle_sse_response(self, response: aiohttp.ClientResponse, request_id: Any) -> Dict[str, Any]:
    ...
    async def forward_request(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.session or not self.mcp_session_id:
                logger.info("Session or mcp_session_id missing/invalid, (re-)initializing...")
            ...
            if not self.mcp_session_id:
                ...
                logger.info(f"Emergency MCP Session ID generated: {self.mcp_session_id}")
            ...
            logger.info(f"Full request payload to be sent: {json.dumps(request)}") # Log the entire request
    ...
    async def handle_stdio(self):
        ...
        logger.info("MCP Stdio Wrapper started")
        ...
        try:
            while True:
                try:
                    ...
                    if not self.session or not self.mcp_session_id:
                        logger.info("Wrapper (MCPStdioWrapper) not fully initialized. Calling initialize().")
                        await self.initialize()
                    
                    # Forward the request. forward_request will handle mcp_session_id injection.
                    logger.debug("Forwarding request to HTTP server...")
                    response = await self.forward_request(request)
                    
                    # Send response back via stdout
                    logger.debug("Sending response back to stdout...")
                    
                    original_request_id = request.get("id")

                    if original_request_id is not None:
                        # This was a request, so Claude Desktop expects a response.
                        response_to_send = {"jsonrpc": "2.0", "id": original_request_id}
                        ...
                        if isinstance(response, dict) and 'error' in response and response['error'] is not None:
                            ...
                            logger.error(f"Ambiguous or malformed response from forward_request for request ID {original_request_id}: {response}")
                            response_to_send['error'] = {
                                "code": -32603, # Internal error
                        ...
                        logger.info(f"Sent response for: {request.get('method', 'unknown')} (ID: {original_request_id})")
                    else:
                        ...
                        logger.info(f"Forwarded notification '{request.get('method', 'unknown')}' to server. Server's raw reaction (if any, via forward_request): {response}")
            ...
            await self.cleanup()
            logger.info("MCP Stdio Wrapper stopped")
