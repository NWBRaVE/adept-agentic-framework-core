# Agentic Framework Tutorial Outline
...
This chapter focuses on deploying the Agentic Framework to a Kubernetes cluster using the provided Helm chart. This marks a significant step towards a production-ready, configurable, and repeatable deployment strategy.

### Infrastructure as Code (IaC) with Helm

The framework now includes a comprehensive Helm chart located in `infra/helm/agentic-framework`. This chart manages the deployment of all the framework's microservices, including their configurations, services, and persistent volumes.
