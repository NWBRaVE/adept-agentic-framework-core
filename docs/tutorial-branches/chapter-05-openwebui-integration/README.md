...
4.  **Langchain Tool Wrappers (`agentic_framework_pkg.scientific_workflow.mcp_langchain_tools`)**:
...
5.  **Tool Execution Layer (MCP Servers)**:
...
6.  **Individual Tools (`agentic_framework_pkg.mcp_server.tools` and `agentic_framework_pkg.hpc_mcp_server.tools`)**:
...
7.  **LLM Interaction (LLM Agnostic Layer - `agentic_framework_pkg.core.llm_agnostic_layer`)**:
...
8.  **Data Persistence & State Management (`agentic_framework_pkg.mcp_server.state_manager`)**:
...
9.  **Logging (`agentic_framework_pkg.logger_config`)**:
...
3.  **Connect OpenWebUI to the Agent Backend:**
    ...
    *   Under "Connect to an OpenAI-compatible API," add your agent's backend URL. By default, this is `http://host.docker.internal:8083`. *Note: On Linux, you may need to use `http://172.17.0.1:8083` or your machine's IP address instead of `host.docker.internal`.*
    *   Go back to the main chat screen, select your new custom model from the model selection dropdown, and start a conversation.
...
Deploying the agentic framework to a Kubernetes cluster (both local, like Minikube/Docker Desktop, and remote cloud-based clusters) involves containerizing the services and defining Kubernetes resources to manage them.

#### Prerequisites

*   A running Kubernetes cluster.
*   `kubectl` configured to communicate with your cluster.
*   Docker (for building and pushing images to a registry).
*   A container registry (like Docker Hub, GHCR, ECR, GCR) that your Kubernetes cluster can access.

#### Configuration

1.  **Container Images**: Before deploying, you need to build and push the Docker images for each service (`mcp_server`, `hpc_mcp_server`, `sandbox_mcp_server`, `streamlit_app`, `openwebui_backend`) to your container registry.
    *   Update the `image` field in your Kubernetes deployment files to point to your pushed images.

2.  **Secrets**: API keys and other sensitive credentials should not be hardcoded. Create a Kubernetes Secret to store them.
    ```bash
    kubectl create secret generic agentic-framework-secrets \
      --from-literal=OPENAI_API_KEY='your-key' \
      --from-literal=ANTHROPIC_API_KEY='your-key' \
      # ... add other secrets from your .env file
    ```
    Your deployment manifests should then reference these secrets.

#### Deployment

The framework can be deployed using a set of Kubernetes manifest files (e.g., located in a `kubernetes/` directory). These files would typically include:

*   `namespace.yaml`: To create a dedicated namespace.
*   `pvc.yaml`: PersistentVolumeClaims for `data` and `shared_uploads`.
*   `deployment.yaml`: For each service (`mcp_server`, `streamlit_app`, etc.), defining replicas, container images, environment variables, and volume mounts.
*   `service.yaml`: To expose the services within the cluster (e.g., using ClusterIP).
*   `ingress.yaml`: (Optional) To expose services like the Streamlit app or OpenWebUI backend to the outside world.

To deploy, you would apply these manifests:
```bash
