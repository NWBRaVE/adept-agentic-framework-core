...
def generate_unique_filename(original_filename: str, prefix: str) -> str:
    """Generates a unique and sanitized filename for shared storage."""
    safe_chars = set("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789._-")
    sanitized_base = "".join(c if c in safe_chars else '_' for c in os.path.splitext(original_filename)[0])
    extension = os.path.splitext(original_filename)[1]
    unique_part = str(uuid.uuid4())[:8]
    session_part = st.session_state.mcp_session_id[:8]
    return f"{prefix}_{sanitized_base}_{session_part}_{unique_part}{extension}"

def save_file(uploaded_file, destination_path: str) -> bool:
    """
    Saves a Streamlit UploadedFile to a destination path.
    Returns True on success, False on failure.
    """
    try:
        with open(destination_path, "wb") as f:
            f.write(uploaded_file.getvalue())
        logger.info(f"Successfully saved '{uploaded_file.name}' to '{destination_path}'")
        return True
    except Exception as e:
        st.error(f"Error saving file to {destination_path}: {e}")
        logger.error(f"Error saving file to {destination_path}", exc_info=True)
        return False
    ...
    with st.expander("document RAG Tool Test"):
        ...
        if uploaded_csv_file is not None:
            ...
            if st.button("Process Uploaded document for RAG", key="process_csv_button_main_app"):
                final_sanitized_filename = generate_unique_filename(uploaded_csv_file.name, prefix="doc")
                secure_shared_file_path = os.path.join(SHARED_UPLOAD_DIR, final_sanitized_filename)

                if save_file(uploaded_csv_file, secure_shared_file_path):
                    st.info(f"File saved to shared volume. Path for tool: {secure_shared_file_path}")
                    async def register_csv_async_rag():
                        try:
                            # Use the standardized URL for the default MCP server
                                    ...
                                    if hasattr(first_content_item, 'text') and isinstance(first_content_item.text, str):
                                        try:
                                            ...
                                            if isinstance(parsed_data, dict) and "file_id" in parsed_data:
                                                ...
                                                st.success(f"document '{uploaded_csv_file.name}' registered with file_id: {parsed_data['file_id']}")
    ...
    with st.expander("Video Transcription & Summarization (HPC)"):
        ...
        if video_input_type == "URL":
            ...
            if uploaded_video_file is not None:
                ...
                final_sanitized_filename = generate_unique_filename(uploaded_video_file.name, prefix="video")
                ...
                if save_file(uploaded_video_file, host_path_for_saving):
