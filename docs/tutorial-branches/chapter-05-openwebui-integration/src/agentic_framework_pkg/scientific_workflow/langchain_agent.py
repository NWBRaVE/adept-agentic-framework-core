...
class ScientificWorkflowAgent:
    def __init__(self, mcp_session_id: Optional[str] = None):
        ...
        try:
            logger.info(f"Initializing ScientificWorkflowAgent's LLM via LLMAgnosticClient using {os.getenv('LANGCHAIN_LLM_MODEL')} for purpose {'agent_main'}")
            ...
            self.default_model_name = os.getenv("LANGCHAIN_LLM_MODEL") # Assume this correctly reflects the model used if the initialization is successful
    ...
    async def arun(self, user_input: str, chat_history: Optional[List[Dict[str, str]]] = None, callbacks: Optional[List[Union[BaseCallbackHandler, AsyncCallbackHandler]]] = None, recursion_limit: int = 25) -> Dict[str, Any]:
        ...
        try:
            ...
            if final_state_result and final_state_result.get('messages'):
                ...
                # Check the last tool message for plot URLs
                for message in reversed(final_state_result["messages"]):
                    if isinstance(message, ToolMessage) and message.name == "ExecuteCode":
                        try:
                            tool_output = json.loads(message.content)
                            if isinstance(tool_output, dict) and tool_output.get("plots"):
                                # Extract the URL for Markdown rendering
                                plot_urls = [p.get('plot_url') for p in tool_output["plots"] if p.get('plot_url')]
                                markdown_images = "\n".join([f"![Generated Plot]({url})" for url in plot_urls])
                                # Prepend the markdown to the final text answer
                                output_content = f"{markdown_images}\n\n{output_content}"
                                logger.info(f"Agent formatted plot URLs into Markdown: {output_content}")
                        except (json.JSONDecodeError, TypeError):
                            continue # Ignore if content is not valid JSON
                        break # Stop after checking the last tool call
    ...
    def get_tools(self) -> List[Any]:
        ...
        This can be useful for debugging or when you need to know which tools the agent can use.
        """
        return self.tools
    
    def get_llm(self):
        """
        Returns the LLM instance used by this agent.
        This is useful for debugging or when you need to access the LLM directly.
        """
        return self.llm
    
    def get_default_model_name(self):
        """
        Returns the default model name used by this agent.
        This is useful for debugging or when you need to know which model the agent is configured to use.
        """
        return self.default_model_name or "No model set (LLMAgnosticClient failed to initialize LLM)."  
